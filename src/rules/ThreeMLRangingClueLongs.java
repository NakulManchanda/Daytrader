...
/**
 * This is the CLUE version of the threeM ranging rule to use for LONGS when using the real time 5 sec bars
 * @author Roy
 */
public class ThreeMLRangingClueLongs extends ThreeMLRanging {
    ...
    public boolean isIn3MLRange(BaseGraph<AbstractGraphPoint> dataGraph) {
        ...
        if (0 < dataGraph.size()) {
            ...
            if (null != atrClassValue) {
                ...
                double lowPrice = lowPoint.getLow();
                if (lowPrice <= upperBound && lowPrice >= lowerBound) {
                    result = true;
                }
            }
        }
