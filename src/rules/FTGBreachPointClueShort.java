...
/**
 * This is the FTGBreach point code to use for the 5 sec bars (SHORTS)
 * @param <T> - Any Price / Time data point
 * @author Roy
 */
public class FTGBreachPointClueShort<T extends AbstractGraphPoint> extends FTGBreachPoint<T>  {
...
//                }
            ...
            if (null != data.getPutup() && null == result) {
                ...
                Double limit = objHigh.getHigh() - (objHigh.getHigh() * overrideFTGFraction);
//                System.out.println("Limit = " + limit + ", Low Price = " + objLow.getHigh() + " High point = " + objHigh.toString() + " Low Point = " + objLow.toString());
                if (limit >= objLow.getHigh()) {
                    result = objLow;
                }
            }
        }
